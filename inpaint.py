...
parser.add_argument("--outw", type=int, default=resolution)
parser.add_argument("--outh", type=int, default=resolution)
...
def read_frame_from_videos(vname):
    ...
    while success:
        ...
        frames.append(image.resize((resolution ,resolution )))
        success, image = vidcap.read()
        count += 1    
    return frames 
...
def main_worker():
    ...
    model = InpaintGenerator(output_size=(60, 60)).to(device)
    ...
    masks = read_frame_from_videos(args.mask)
    ...
    writer = cv2.VideoWriter(f"result.mp4", cv2.VideoWriter_fourcc(*"mp4v"), default_fps, (args.outw, args.outh))
    ...
    writer.release()
    print('Finish in {}'.format(f"result.mp4"))
